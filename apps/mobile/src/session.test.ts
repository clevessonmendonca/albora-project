...
describe("resgate do código de 4 dígitos", () => {
  it("aceita o corpo que o handler promete ao nativo", () => {
    ...
    ).toEqual({ token: "abc.def", slug: "festa-demo", sessaoId: "sess-1", eventoId: "ev-1" });
  });

  it("recusa resposta sem token — cookie HttpOnly não chega no app", () => {
    expect(
      parseRedeemResponse({ slug: "festa-demo", sessaoId: "sess-1", eventoId: "ev-1" }),
    ).toBeNull();
  });

  it("recusa resposta sem eventoId — a fila é por evento", () => {
    expect(
  ...
  it("sessão persistida sem eventoId não abre a câmera", () => {
    expect(
      parseStoredSession(JSON.stringify({ token: "abc.def", slug: "festa-demo", sessaoId: "sess-1" })),
    ).toBeNull();
