...
export type NormalizeOpts = {
  /**
   * Cabeçalho (≥12 bytes) do arquivo para detecção de formato.
   * Ignorado quando `alwaysConvert` for true.
   */
  head: Uint8Array;
  ...
  height?: number;
  /**
   * Quando true, converte para JPEG independente do formato detectado.
   * Use para URIs de galeria (ph://, content://) onde leitura direta pode falhar.
   */
  alwaysConvert?: boolean;
  /**
   * Injeção para testes: substitui `manipulateAsync` do expo-image-manipulator.
   * Recebe a URI e devolve a URI do JPEG produzido.
   */
...
/**
 * Normaliza uma imagem da galeria ou de câmera para JPEG.
 * Converte quando:
 *   (a) o cabeçalho indica HEIC, ou
 *   (b) alwaysConvert é true (padrão para URIs de galeria).
 * Retorna CaptureSource pronto para persistCapture.
 */
