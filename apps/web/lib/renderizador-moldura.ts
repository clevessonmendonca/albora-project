...
function corCss(nome: string, fallback: string): string {
...
function desenharFaixaConteudo(
  ...
  const superficie = corCss("--superficie", "#f5f0ea");
  const ink = corCss("--ink", "#1a1512");
  const ink2 = corCss("--ink-2", "#5c534c");
  const acento = corCss("--acento", "#c45c26");
  const fonteTitulo = corCss("--fonte-titulo", "Georgia, serif").replace(/"/g, "");
  const fonteCorpo = corCss("--fonte-corpo", "system-ui, sans-serif").replace(/"/g, "");
...
export async function desenharMoldura(
  imagem: CanvasImageSource,
  composicao: Composicao,
): Promise<Blob> {
  ...
  ctx.fillStyle = corCss("--bg", "#faf7f2");
...
export async function desenharColagem(
...
export async function compartilharOuBaixar(blob: Blob, nomeArquivo: string) {
