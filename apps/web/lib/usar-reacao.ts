...
import { drenarReacoesPendentes, enfileirarReacao, type AcaoReacao } from "./fila-interacao";
...
async function enviarReacao(acao: AcaoReacao): Promise<boolean> {
  try {
    const r = await fetch("/api/reacao", {
...
export function usarReacao(
  ...
  const drenar = useCallback(async () => {
    ...
    try {
      await drenarReacoesPendentes(enviarReacao);
    } finally {
