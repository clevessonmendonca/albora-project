...
function abrir(): Promise<IDBDatabase> {
  return new Promise((ok, falha) => {
    ...
    try {
      ...
      falha(new QueueUnavailableError(e));
      return;
    }
    ...
    req.onerror = () => falha(new QueueUnavailableError(req.error));
    req.onblocked = () => falha(new QueueUnavailableError("outra aba segurando a versão antiga"));
  });
}
    ...
    (db) =>
      new Promise<T>((ok, falha) => {
        ...
        tx.onabort = () => {
          ...
          falha(tx.error?.name === "QuotaExceededError" ? new QueueQuotaExceededError() : tx.error);
        };
        tx.onerror = () => {
          db.close();
          falha(tx.error);
