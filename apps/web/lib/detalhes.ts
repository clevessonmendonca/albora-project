 ...
 */
export function legendaLimpa(valor: unknown): string | null {
  ...
  const limpa = valor.replace(/[\p{Cc}\p{Cf}]/gu, " ").trim().slice(0, MAX_LEGENDA);
 ...
 */
export function lugarAceito(packId: string | null, valor: unknown): string | null {
