...
export type UrlDeMidia = { chave: string; url: string; expiraEm: number };
 ...
 * resolve nada.
 */
export async function urlsDeMidia(chaves: string[]): Promise<Map<string, UrlDeMidia>> {
  if (chaves.length === 0) return new Map();

  const res = await fetch("/api/midia/urls", {
    method: "POST",
  ...
  if (!res.ok) throw new ErroDeMidia(res.status);

  const corpo = (await res.json()) as { urls: UrlDeMidia[] };
...
export function expirou(url: UrlDeMidia, agora: number): boolean {
  return agora >= url.expiraEm - FOLGA_DE_RENOVACAO_MS;
}
