...
import { agruparPorHora, rotuloDeHora } from "./agrupar-por-hora";
...
describe("agrupar por hora", () => {
  ...
  it("entrega os grupos do mais recente para o mais antigo", () => {
    const grupos = agruparPorHora([item("a", em(21, 0)), item("b", em(23, 0)), item("c", em(22, 0))], {
      temMais: false,
    });
  ...
  it("dentro do grupo a hora corre para frente, mesmo com a entrada fora de ordem", () => {
    const grupos = agruparPorHora([item("c", em(23, 50)), item("a", em(23, 1)), item("b", em(23, 30))], {
      temMais: false,
    });
  ...
  it("23h de dois dias são duas horas diferentes", () => {
    const grupos = agruparPorHora([item("sabado", em(23, 10, 9)), item("domingo", em(23, 10, 10))], {
      temMais: false,
    });
  ...
  it("aceita o instante como string, que é o que chega pela rede", () => {
    const grupos = agruparPorHora([item("a", em(20, 15).toISOString())], { temMais: false });
  ...
  it("descarta instante ilegível em vez de criar um grupo inválido", () => {
    const grupos = agruparPorHora([item("bom", em(21, 0)), item("torto", "nem data é")], {
      temMais: false,
    });
  ...
  it("com página pendente, só a hora mais antiga fica incompleta", () => {
    const grupos = agruparPorHora([item("a", em(23, 0)), item("b", em(22, 0)), item("c", em(21, 0))], {
      temMais: true,
    });
  ...
  it("sem página pendente, toda hora está fechada", () => {
    const grupos = agruparPorHora([item("a", em(23, 0)), item("b", em(22, 0))], { temMais: false });

    expect(grupos.every((g) => g.completo)).toBe(true);
  });

  it("o início da hora zera minuto, segundo e milissegundo", () => {
    const grupos = agruparPorHora([item("a", em(23, 47))], { temMais: false });
  ...
  it("lista vazia não vira grupo vazio", () => {
    expect(agruparPorHora([], { temMais: false })).toEqual([]);
    expect(agruparPorHora([], { temMais: true })).toEqual([]);
  });

  it("não muta a lista recebida", () => {
    const entrada = [item("a", em(21, 0)), item("b", em(23, 0))];
    agruparPorHora(entrada, { temMais: false });
...
describe("rótulo da hora", () => {
  it("é numeral arábico com dois dígitos", () => {
    expect(rotuloDeHora(23)).toBe("23h");
    expect(rotuloDeHora(9)).toBe("09h");
    expect(rotuloDeHora(0)).toBe("00h");
  });
});
