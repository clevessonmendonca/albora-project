...
export function identidadeParaMoldura(
  slug: string,
  comecaEm: Date,
  ...
  const monograma =
    ...
    camada.monograma ??
    (pack ? texto(pack, "landing.exemplo.nome").slice(0, 2).toUpperCase() : slug.slice(0, 2).toUpperCase());

  const titulo =
    camada.texto?.titulo ??
    camada.titulo ??
    (pack ? texto(pack, "landing.exemplo.nome") : slug);
