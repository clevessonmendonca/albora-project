...
import { urlsDeMidia } from "@/lib/midia";
import {
  carregarImagem,
  compartilharOuBaixar,
  desenharColagem,
  desenharMoldura,
} from "@/lib/renderizador-moldura";
...
export function usarCompartilhar(eventoId: string, sessaoId: string) {
  ...
  const buscarContexto = async (uploadId: string): Promise<ContextoApi> => {
    const r = await fetch(`/api/compartilhar?uploadId=${uploadId}`, {
  ...
  const executar = useCallback(
    async (uploadId: string, consentimentoExterno: ConsentimentoExterno | null) => {
      ...
      try {
        ...
        const blob = await desenharMoldura(img, resultado.composicao);
        await compartilharOuBaixar(blob, `albora-${ctx.evento.slug}.jpg`);
      } catch {
  ...
  const executarColagem = useCallback(
    async (uploadIds: string[], consentimentoExterno: ConsentimentoExterno | null) => {
      ...
      try {
        ...
        const urls = await urlsDeMidia(chaves);
        ...
        const blob = await desenharColagem(fotos, conteudo);
        await compartilharOuBaixar(blob, `albora-${base.evento.slug}-colagem.jpg`);
      } catch {
  ...
  const confirmarConsentimento = useCallback(
    async (uploadId: string, nomeNaMoldura: boolean) => {
      setErro(null);
      try {
        const r = await fetch("/api/compartilhar", {
          method: "POST",
