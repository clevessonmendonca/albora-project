...
import { transporteWeb, ErroDeApi } from "./transporte";
...
describe("transporteWeb", () => {
  ...
  it("presign envia metadados do item", async () => {
    ...
    const resposta = await transporteWeb.presign(itemBlob);
  ...
  it("confirmar propaga codigo de erro da API", async () => {
    ...
    await expect(
      transporteWeb.confirmar(itemBlob, {
        uploadId: itemBlob.id,
        chave: "events/e/full",
        full: "u",
        thumb: "t",
...
describe("ErroDeApi", () => {
  it("marca 401, 403 e 422 como definitivos", () => {
    expect(new ErroDeApi("presign", 401).definitivo).toBe(true);
    expect(new ErroDeApi("put", 403).definitivo).toBe(true);
    expect(new ErroDeApi("confirm", 422).definitivo).toBe(true);
  });

  it("deixa 503 retentável", () => {
    expect(new ErroDeApi("put", 503).definitivo).toBe(false);
  });
});
