...
/**
 * ≥44px de alvo de toque para os botões de texto desta tela — mesmo padrão de
 * host-album.tsx/review-queue.tsx (`min-h-11` independente do padding vertical da cascata).
 */
      ...
      <label className="flex flex-col gap-1.5">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`tipo-caption inline-flex ${ALVO_TOQUE} cursor-pointer items-center border-none bg-transparent p-0 text-acento-texto transition-opacity duration-[var(--tempo-rapido)] ease-[var(--curva)] hover:opacity-70 disabled:cursor-default disabled:opacity-50`}
          >
        ...
        <input
          ...
          className="tipo-body w-full rounded-token border border-linha bg-bg px-3.5 py-[0.65rem] text-ink outline-none transition-[border-color] duration-[var(--tempo-rapido)] ease-[var(--curva)] focus:border-acento"
        />
      </label>
      ...
      <div className="mt-2 grid gap-2">
        ...
        {loading ? (
          ...
          <ul className="m-0 grid list-none gap-2 p-0">
            {suggestions.map((s) => (
              <li key={`${s.provedor}:${s.tipo}:${s.url}`} className="tipo-body flex items-center justify-between gap-3 rounded-token bg-bg px-3 py-2.5 text-ink">
