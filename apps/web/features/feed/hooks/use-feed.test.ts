...
import { RENEWAL_BUFFER_MS, type MediaUrl } from "@/lib/media";
import {
  buscarPagina,
  chavesSemUrl,
  comFalha,
...
const url = (chave: string, expiraEm: number, valor = `https://r2/${chave}`): MediaUrl => ({
  chave,
  url: valor,
  expiraEm,
});
...
describe("URLs de mídia em lote", () => {
  ...
  it("renova antes de a URL vencer, não no instante", () => {
    ...
    const quaseVencida = agora + RENEWAL_BUFFER_MS - 1;
    const e = comUrls(
      comItens(["a"]),
