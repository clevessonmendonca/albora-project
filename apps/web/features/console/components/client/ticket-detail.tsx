...
import { RotuloSerif } from "@/features/console/components/server/console-primitivos";
  ...
  return (
    <div className="flex flex-col gap-5">
      <header className="flex flex-col gap-4 border-b border-linha pb-5">
        <div>
          <h2 className="tipo-den-titulo m-0 text-ink">{ticket.subject}</h2>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_14rem]">
        <section className="flex flex-col gap-3">
          <h3 className="m-0">
            <RotuloSerif>Conversa</RotuloSerif>
          </h3>
          {messages.length === 0 ? (
            ...
            <ul className="m-0 flex list-none flex-col gap-2 p-0">
              {messages.map((m) => (
                <li key={m.id} className="rounded-token border border-linha p-3">
                  ...
                  <p className="tipo-den-corpo m-0 mt-2 text-ink">{m.body}</p>
                </li>
              ))}
            </ul>
          )}
        ...
        <section className="flex flex-col gap-3 lg:border-l lg:border-linha lg:pl-6">
          <h3 className="m-0">
            <RotuloSerif>Contexto do cliente</RotuloSerif>
          </h3>
          <dl className="m-0 flex flex-col gap-2.5">
            <div className="flex items-center justify-between gap-3 border-b border-linha pb-2">
              <dt className="tipo-den-meta text-ink-3">Plano</dt>
              <dd className="tipo-den-corpo m-0 text-ink">{customerContext.plan ?? "—"}</dd>
            </div>
            <div className="flex items-center justify-between gap-3 border-b border-linha pb-2">
              <dt className="tipo-den-meta text-ink-3">Eventos</dt>
              <dd className="tipo-den-dado m-0 text-ink">{customerContext.events.length}</dd>
            </div>
            <div className="flex items-start justify-between gap-3">
              <dt className="tipo-den-meta shrink-0 text-ink-3">Pagamentos recentes</dt>
              <dd className="tipo-den-corpo m-0 text-right text-ink">
