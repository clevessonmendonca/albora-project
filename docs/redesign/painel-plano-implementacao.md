...
- [ ] **4. Adoção do Design System** *(nova, pré-requisito das demais)* — eliminar `adminClasses` e as classes soltas da Home; admin passa a consumir `Button`, `Card`, `PageHeader`, `Skeleton`, `EmptyState`. Critério: nenhuma string de botão fora do DS; visual inalterado ou melhor.
