...
- [x] **9. Depois + retenção visível** — payoff com fotos/pessoas/favoritas, **"novas para você"** (`events.host_seen_album_at`, carimbado ao abrir o álbum), Guardar e **timeline de retenção** com os marcos reais dos jobs (`marcosDeRetencaoDoEvento`), caindo para a data calculada quando o job ainda não foi agendado. **Não feito, decisão pendente**: cápsula de memória — guardar destaques além do dia 365 mexe numa promessa jurídica sobre foto de convidado, não é escolha de implementação. Momentos continua sendo o link para Insights, não a grade por capítulo do protótipo.
