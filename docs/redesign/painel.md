...
- **Fotos / Álbum + Moderação**: grade; **ocultar/liberar/remover** (existe); **fila de denúncias** com contagem (existe); comentários (existe); **falta "destacar"** (curadoria pro álbum/telão) — adicionar. Seleção pro álbum-livro.
- **Convidados**: funil de participação (existe) + pessoas (quem mais registrou, primeira foto) — só com dado real, menos CRM.
