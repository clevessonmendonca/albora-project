...
- 🟠 **Só o `full` passa por magic bytes.** O `confirm` faz `Range` GET dos primeiros 16 bytes de `.../full` e recusa declaração que os bytes desmintam (`midia.conteudo_nao_confere`) **antes** de persistir — não é checagem só no cliente, nem a heurística do classificador na thumb. A miniatura sobe pela segunda URL assinada e nunca é lida neste portão. O feed devolve a chave da miniatura, e a leitura assina as duas variantes — existe, portanto, objeto servível cujo conteúdo nunca passou pelo controle 5 da §4.2. Quem segura a thumb hoje é a separação de origem, não a validação. Uma segunda leitura no caminho crítico de sábado foi recusada de propósito; a checagem da miniatura continua no item 9 da §10.
