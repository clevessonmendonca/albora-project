...
- ✅ **`full` e `thumb` passam por magic bytes.** O `confirm` faz `Range` GET dos primeiros 16 bytes de `.../full` e, após validar, faz o mesmo para `.../thumb` (sempre exigindo `image/jpeg` — incluindo poster de vídeo). Ambos são recusados se os bytes desmentirem o MIME declarado (`midia.conteudo_nao_confere`) **antes** de persistir. Thumb ausente retorna 409 `upload.thumb_ausente`.
