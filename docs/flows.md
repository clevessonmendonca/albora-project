...
**Parear ✅:** index redireciona com sessão válida → feed; deep link `albora://pair?codigo=1234` preenche e **resgata**; `?passagem=` resgata direto; Expo Router `/pair?…` (Redirect de `app/e/[slug]/pair`) também auto-resgata via `useLocalSearchParams`.
