...
**Compartilhar ✅:** em `photo-detail` e em Minhas (foto enviada) → `GET/POST /api/share` + composição `compor` (`@albora/core`) + moldura Skia 9:16 (`share-skia-frame.ts`) + `expo-sharing`. Se Skia/compose falhar, cai no arquivo raw (nunca bloqueia). Vídeo usa `chaveThumb`. Tipografia da moldura: Fraunces/Instrument Sans **embutidas** (`share-font-registry` + `assets/fontes/` via `copiar-fontes.mjs`); fallback `matchFont` do SO para famílias custom do casal.
