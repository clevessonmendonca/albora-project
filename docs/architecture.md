...
5. **`confirm` valida, não confia.** Verifica que o objeto `/full` existe, que a chave começa com o prefixo do evento da sessão, e que os primeiros bytes são de fato do tipo declarado (JPEG, PNG, WEBP ou mp4). O `Content-Type` é declarado pelo cliente e não vale nada: um "JPEG" que é HTML servido da origem do app é XSS armazenado com alcance de festa inteira. A leitura no storage é por `Range` nos primeiros 16 bytes **do `/full`** — o suficiente para a assinatura de arquivo, e nada perto de trafegar a foto pelo servidor. A checagem no cliente e a heurística do classificador na thumb não substituem este portão.
...
**Experiência do convidado v4.** A **chegada** é emocional: capa full-bleed (o mesmo GET assinado da rota `/cover`, ou o chão do evento tingido quando não há foto) + "Você foi convidado para {nome}" + "Entrar na festa"; o nome real do evento (`events.title`), não o exemplo do pack. O **recado dos anfitriões** deixou de ser card e virou o **primeiro story** do trilho da home (anel especial + selo de áudio, abre em tela cheia com o player de voz), reusando `/api/recado`. A **identidade tardia** — feed antes de nome/consentimento, identidade pedida só na 1ª ação via `IdentitySheet` — é hipótese A/B atrás da flag `lateIdentity` (off por padrão): o consentimento versionado continua **antes de qualquer captura** (a folha o coleta na 1ª publicação/reação), só a chegada deixou de exigi-lo. Ligar a flag depende de o feed servir uma **leitura-espelho sem sessão** (peça do domínio do feed), especificada em [ADR 0021](./adr/0021-identidade-tardia-do-convidado.md), ainda não construída; até lá o `EntryFlow` (com a chegada nova) é o caminho.
...
| App nativo Expo | ADR 0008–0010, spec 017 | **Parcial** — câmera, fila, drain, feed, PUT background, Desenhista + tira de presets na revisão; falta preview ao vivo do filtro, paridade de telas, lojas/EAS |
